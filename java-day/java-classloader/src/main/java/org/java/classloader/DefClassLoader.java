...
public class DefClassLoader extends ClassLoader {
	...
	public Class<?> loadClass(String name) throws ClassNotFoundException {
		 //查找已经加载的类
		 //hotstop 使用hash表维护已经加载的类
		 //判断是同一个Class对象，name值相等，classloader对象地址相等
		 //具体看 org.java.jvm.classfile.Dictionary
		 findLoadedClass(name);
		return super.loadClass(name);
	}
	 @Override
	protected Class<?> findClass(String name) throws ClassNotFoundException {
		 //定位class文件的位置读取 成 byte[]
		// 利用  classloader 方法 
	 ...
	 //ClassLoader 不实现由具体的类实现
	 //ext,system 类实现了URLClassLoader urlClasspath维护
	 @Override
